...
def create_animation(scenario, planning_problem_set, time):
    ...
    rnd.create_video([scenario, planning_problem_set], f"car/animation/{str(scenario.scenario_id)}_uppaal.gif", draw_params=dp)
'''    
def create_animation2(scenario, planning_problem_set, time):
    ...
    gif_path = os.path.join(root, f"car/animation/{str(scenario.scenario_id)}_uppaal.gif")
...
def generate(SCENARIO_PATH, save=False):
    ...
    if(save):
        ...
        new_scenario_path = f"{base}-shielded{ext}"
        save_scenario(scenario, planning_problem_set, new_scenario_path)

    # Create output folder for figures
    output_fig_path = os.path.join(root, "animation/figures")
