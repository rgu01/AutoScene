...
def calculate_travel_time(lanelet):
...
for xml_file in os.listdir(input_file_folder):
    ...
    for i, lane in enumerate(scenario.lanelet_network.lanelets):
        #
        lane_length = (calculate_travel_time(lane)*pow(BASE,EXPONENT)).astype(int)
    ...
    THRES_COL_str = f"const double THRESHOLD_COLLISION = {THRESHOLD_COLLISION};"
    THRES_REC_str = f"const double THRESHOLD_REACH = {THRESHOLD_REACH};"
