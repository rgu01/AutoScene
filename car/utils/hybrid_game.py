...
class HybridGame:
    ...
    def generate_uppaal_model(self):
        ...
        MAXP = max(len(lane._left_vertices) for lane in self.scenario.lanelet_network.lanelets)
        # MAXL is the number of lanes in the lanelet network
        MAXL = len(self.scenario.lanelet_network.lanelets)
        # MAXSO is the number of static obstacles
        MAXSO = max(1, len(self.scenario.static_obstacles))
        # MAXDO is the number of dynamic obstacles
        MAXDO = max(1, len(self.scenario.dynamic_obstacles))
        # MAXTP is the number of max points in trajectories of moving obstacles
        MAXTP = 0 if self.scenario.dynamic_obstacles == [] else max(len(dyn_obs.prediction.trajectory.state_list) for dyn_obs in self.scenario.dynamic_obstacles)
        # MAXPRE is the maximal number of predecessor lanes 
        MAXPRE = max(1, max(len(lane.predecessor) for lane in self.scenario.lanelet_network.lanelets))
        # MAXSUC is the maximal number of successor lanes 
        MAXSUC = max(1, max(len(lane.successor) for lane in self.scenario.lanelet_network.lanelets))
        # TIMESTEPSIZE is the step time size
        TIMESTEPSIZE = self.scenario.dt
        ...
        ST_LANE_laneNet_str = []
        for i, lane in enumerate(self.scenario.lanelet_network.lanelets):
            #
            travel_time = self.calculate_travel_time(lane)
            lane_length = (travel_time*pow(self.config.BASE,self.config.EXPONENT)).astype(int)
            ...
            leftLane = (lane._left_vertices*pow(self.config.BASE,self.config.EXPONENT)).astype(int)
            rightLane = (lane._right_vertices*pow(self.config.BASE,self.config.EXPONENT)).astype(int)
            # merge the lane points if they are on the same straight line 
            ...
            ST_LANE_lane = f"const ST_LANE lane{i + 1} = " + "{" + ", ".join([f"{lane_ID}", f"leftLane{i + 1}", f"rightLane{i + 1}", 
                                                                            ...
                                                                            f"{lane_dirLeft}", f"{lane_adjRight}", f"{lane_dirRight}", f"{lane_length}"]) + "};"
        ...
        for static_obs in self.scenario.static_obstacles:
            obs_pos = (static_obs._initial_state.position*pow(self.config.BASE,self.config.EXPONENT)).astype(int)
            obs_ori = int(static_obs._initial_state.orientation*pow(self.config.BASE,self.config.EXPONENT))
            obs_width = int(static_obs._obstacle_shape._width*pow(self.config.BASE,self.config.EXPONENT))
            obs_length = int(static_obs._obstacle_shape._length*pow(self.config.BASE,self.config.EXPONENT))
        ...
        if len(self.scenario.obstacles) == 0:
            ST_RECTANGLE_obs_str1 = "const bool dynamicObsExists = false;\n"
        else:
            ST_RECTANGLE_obs_str1 = "const bool dynamicObsExists = true;\n"
        ...
        for dyn_obs in self.scenario.dynamic_obstacles:
            obs_width = int(dyn_obs._obstacle_shape._width*pow(self.config.BASE,self.config.EXPONENT))
            obs_length = int(dyn_obs._obstacle_shape._length*pow(self.config.BASE,self.config.EXPONENT))
            ...
            obs_ini_jerk = self.config.DEFAULT_VAL
            ...
            obs_ini_pos_int = "{" + ", ".join(map(str, (obs_ini_pos*pow(self.config.BASE,self.config.EXPONENT)).astype(int))) + "}"
            shapeObs_str = f"const ST_RECTANGLE shapeObs{obs_id} = {{" + ", ".join([obs_ini_pos_int, 
                            f"{int(dyn_obs._obstacle_shape._width*pow(self.config.BASE,self.config.EXPONENT))}", 
                            f"{int(dyn_obs._obstacle_shape._length*pow(self.config.BASE,self.config.EXPONENT))}", 
                            f"{int(dyn_obs._obstacle_shape._orientation*pow(self.config.BASE,self.config.EXPONENT))}"]) + "};"
            ...
            for i in range(MAXTP):
                ...
                if i+1 > len_traj:
                    ...
                    spd = dyn_obs_pos.velocity if hasattr(dyn_obs_pos, 'velocity') else self.config.DEFAULT_VAL
                    acc = dyn_obs_pos.acceleration if hasattr(dyn_obs_pos, 'acceleration') else self.config.DEFAULT_VAL
                    jerk = self.config.DEFAULT_VAL
                    yaw = self.config.DEFAULT_VAL
        ...
        planning_problem = list(self.planning_problem_set.planning_problem_dict.values())
        ...
        for planning_problem_veh in planning_problem:
            try:
                ...
                try:
                    ...
                    sys.exit(1)
            goal_pos = (goal_pos*pow(self.config.BASE,self.config.EXPONENT)).astype(int)
            ...
            ego_ini_jerk = self.config.DEFAULT_VAL
            ...
            INT32_ego_ini_pos = "{" + ", ".join(map(str, (ego_ini_pos*pow(self.config.BASE,self.config.EXPONENT)).astype(int))) + "}"
            ...
            #init_ego_shape_str = "const ST_RECTANGLE initShapeEgo = {{" + ", ".join(map(str, (ego_ini_pos*pow(self.config.BASE,self.config.EXPONENT)).astype(int))) + "}, 100, 450, 0};"
            #init_ego_shape_str = "const ST_RECTANGLE initShapeEgo = {" + ", ".join([INT32_ego_ini_pos, f"{(self.config.EV_WIDTH*pow(self.config.BASE,self.config.EXPONENT)).astype(int)}", 
            #                    f"{(self.config.EV_LENGTH*pow(self.config.BASE,self.config.EXPONENT)).astype(int)}", f"{(self.config.EV_ORI*pow(self.config.BASE,self.config.EXPONENT)).astype(int)}"]) + "};"
            init_ego_shape_str = "const ST_RECTANGLE initShapeEgo = {" + ", ".join([INT32_ego_ini_pos, f"{self.config.EV_WIDTH*pow(self.config.BASE,self.config.EXPONENT)}", 
                                f"{int(self.config.EV_LENGTH*pow(self.config.BASE,self.config.EXPONENT))}", f"{self.config.EV_ORI*pow(self.config.BASE,self.config.EXPONENT)}"]) + "};"
        ...
        P_str = f"const int P = {self.config.P};"
        MAX_TIME_str = f"const uint16_t MAXTIME = {self.config.MAXT};"
        ...
        with open(self.game_file, 'w') as file:
            for line in lines:
                ...
                if line.strip() == scenario_prompt:
                    ...
                    file.write(THRES_COL_str + "\n")
                    file.write(THRES_REC_str + "\n")
